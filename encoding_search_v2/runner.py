...
def _stage_circuits_output_dir(config: PipelineConfig) -> Optional[str]:
    if not config.export_circuits:
        return None
    return os.path.join(
        stage_output_dir(config.state_name, config.stage, output_root=config.output_root),
        "circuits",
    )
...
def _write_preselected_rows(config: PipelineConfig) -> str:
    ...
    output_dir = stage_output_dir(config.state_name, config.stage, output_root=config.output_root)
...
def run_stage1(config: PipelineConfig):
    ...
    paths = write_result_bundle(
        ...
        file_prefix=stage_file_prefix(config.state_name, 1),
        top_k=config.top_k,
...
def run_stage2(config: PipelineConfig):
    ...
    paths = write_result_bundle(
        df,
        output_dir=output_dir,
        file_prefix=stage_file_prefix(config.state_name, 2),
        top_k=config.top_k,
        fidelity_thresholds=config.fidelity_thresholds,
    )
