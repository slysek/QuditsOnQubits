...
def prepare_campaign(artifacts, schedule, adapter, transpilation, instances, seed,
                     *, native_circuits=None, readout_mitigation=True):
    ...
    source = (adapter.compile(prepared.circuits, transpilation).circuits
              if native_circuits is None else native_circuits)
    compiled = tuple(physical_isa(circuit) for circuit in source)
    ...
    suppressed_arm = 'MM_TWIRL_DD' if readout_mitigation else 'TWIRL_DD'
    for arm, factor in [('RAW', 1), *[(suppressed_arm, f) for f in (1, 3, 5)]]:
    ...
    calibrations = {}
    for mapping in sorted(set(mappings)) if readout_mitigation else ():
        circuits = []
...
def execute_campaign(directory, schedule, evidence, batches, calibrations, backend,
                     *, allow_hardware=False, calibration_shots=4096, max_circuits=100,
                     expected_calibration_set_id=None):
    ...
    if (expected_calibration_set_id is not None
            and str(backend.architecture.calibration_set_id) != expected_calibration_set_id):
        raise ValueError('Backend calibration differs from the frozen campaign')
    ...
    write_json(directory / 'backend.json', {
        ...
        'calibration_set_id': expected_calibration_set_id or str(getattr(backend, 'calibration_set_id', '')),
    })
    requests = []
    ...
    for request in requests:
        ...
        result = job.result()
        if expected_calibration_set_id is not None:
            observed = {str(getattr(item, 'calibration_set_id', '')) for item in result.results}
            if observed != {expected_calibration_set_id} or len(result.results) != len(request['circuits']):
                raise ValueError('Result calibration or circuit count differs from the frozen campaign')
            write_json(directory / f'{name}.result.json', {
                'job_id': str(job.job_id()), 'calibration_set_id': expected_calibration_set_id,
                'circuit_count': len(result.results),
            })
...
def analyze_campaign(directory, schedule, metadata):
    ...
    suppressed_arm = 'MM_TWIRL_DD' if matrices else 'TWIRL_DD'
    if {(r['arm'], r['factor']) for r, _ in requests if 'arm' in r} != {
            ('RAW', 1), *[(suppressed_arm, f) for f in (1, 3, 5)]}:
        raise ValueError('Incomplete or inconsistent campaign arms')
    grouped = {}
    ...
    for (arm, factor), blocks in grouped.items():
        ...
        for corrected in ([False, True] if arm == 'RAW' and matrices else [bool(matrices)]):
            pooled = {}
            for block in schedule.blocks:
    ...
    scaled = sorted((r for r in rows if r['variant'] == suppressed_arm), key=lambda r: r['factor'])
    intercept, fit = linear_zne_extrapolate([r['factor'] for r in scaled],
        [complex(r['real'], r['imag']) for r in scaled])
    rows.append({'variant': suppressed_arm + '_ZNE', 'factor': 0, 'real': intercept.real, 'imag': intercept.imag})
